...
if not os.path.isfile("Datos.cvs"):
    with open("Datos.cvs", 'w') as file:
...
ttk.Label(tab4, text="En reparacion").grid(column=0, row=0, padx=10, pady=10)
ttk.Label(tab5, text="En reparacion").grid(column=0, row=0, padx=10, pady=10)
...
cancelar_button = ttk.Button(sub_frame5, text="Cancelar", command=cancelar).grid(column = 1, row=0,padx= 12)
...
# Agregar filas a la tabla con los datos de las personas guardadas
for i, persona in enumerate(personas):
    table.insert(parent="", index=i, iid=i, text=i+1, values=(persona.nombre, persona.a_paterno, persona.a_materno, persona.correo, persona.movil, persona.ocupacion, persona.aficionLeer, persona.aficionMusica, persona.aficionVideojuegos, persona.estado))
